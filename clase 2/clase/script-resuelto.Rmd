...
```{r}
unique(base_cargos_2022$Provincia)
base_cargos_2022 <- base_cargos_2022[base_cargos_2022$Provincia == "Ciudad de Buenos Aires", ]
```
...
```{r}
names(base_cargos_2022)
base_cargos_2022_select <- select(base_cargos_2022, Sector, Nivel, Categoria, Dummy_Cubierto, value)
```

8.b. Filtrar por ámbito estatal y nivel primario y secundario

```{r}
base_cargos_2022_filter <- filter(base_cargos_2022_select, Sector == "Estatal",  Nivel %in% c("Nivel Primario", "Nivel Secundario"), Dummy_Cubierto != "Cubiertos")
```
8.c. Agrupemoslo por nivel educativo:

```{r}
base_cargos_2022_group <- group_by(base_cargos_2022_filter, Nivel) 
```

8.d. Calcular la suma de cargos sin cubrir:
```{r}
base_cargos_2022_sum <- summarise(base_cargos_2022_group, suma_cargos_sin_cubrir = sum(value, na.rm = T))
base_cargos_2022_sum
```
...
```{r}
base_cargos_2022 %>%
  select(Sector, Nivel, Categoria, Dummy_Cubierto, value) %>%
  filter(Sector == "Estatal",  Nivel %in% c("Nivel Primario", "Nivel Secundario"),  Dummy_Cubierto != "Cubiertos") %>%
  group_by(Nivel) %>%
  summarise(suma_cargos_sin_cubrir = sum(value, na.rm = T))
```

## Uso de select:
10.a. Selecciona todo el dataframe menos la columna 'Provincia':
```{r}
base_cargos_2022 %>% select(-Provincia)
```
...
```{r}
base_cargos_2022 %>%
  filter(Nivel == "Nivel Primario" | Sector == "Privado")
```
...
```{r}
tipos_deseados <- c("Direccion", "Apoyo", "Frente a Alumnos")
base_cargos_2022 %>%  filter(Categoria %in% tipos_deseados)
```

11.c. Filtrar aquellos con 'value' es "NA". Reasignarlo posteriormente a nuestra variable "base_cargos_2022":
```{r}
base_cargos_2022 <- base_cargos_2022 %>%
  filter(!is.na(value))
```
...
```{r}
base_cargos_bis_2022_def <- read_csv("base_cargos_bis_2022_def.csv", 
...
```{r}
data_join<-  base_cargos_bis_2022_def %>%
  inner_join(base_cargos_2022, by = c("ID", "Provincia", "Sector", "Ambito", "Nivel", "Categoria", "Dummy_Cubierto"))
```
...
data_rbind<- rbind(base_cargos_bis_2022_rio_negro, base_cargos_bis_2022_def)
data_rbind
```
...
```{r}
mutate_df <- base_cargos_2022 %>%
      filter(!is.na(value)) %>%
      pivot_wider(names_from = Dummy_Cubierto, values_from = value) %>%
      group_by(ID) %>%
      summarise(sum_cubiertos = sum(Cubiertos,na.rm = T), 
                sum_no_cubiertos = sum(`Sin Cubrir`,na.rm = T),
                porcentaje = sum_no_cubiertos / (sum_cubiertos + sum_no_cubiertos)) %>%
      mutate(porcentaje_categorias = case_when(
        porcentaje < 0.25 ~ "Baja",
        porcentaje > 0.26 & porcentaje < 0.50 ~ "Media",
        porcentaje > 0.51  ~ "Alta",
        T~ "Información no disponible"
      ))
...
```{r}
mutate_df %>%
  group_by(porcentaje_categorias) %>%
